    ...
    <div className="p-8 space-y-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="glass-panel p-6 rounded-2xl border border-slate-800 space-y-4">
          <h3 className="text-sm font-semibold text-slate-200">Organization Info</h3>
          <div className="space-y-3 text-xs">
            <div>
              <span className="text-[10px] text-slate-500 font-mono block">Organization Name</span>
              <span className="font-bold text-slate-100">{org.name}</span>
            </div>
            <div>
              <span className="text-[10px] text-slate-500 font-mono block">Slug / Domain</span>
              <span className="font-mono text-cyan-400">{org.slug}</span>
            </div>
            <div>
              <span className="text-[10px] text-slate-500 font-mono block">Industry</span>
              <span className="text-slate-300">{org.industry}</span>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 glass-panel p-6 rounded-2xl border border-slate-800 space-y-4">
          ...
          <div className="space-y-3 pt-2">
            {invitations.map((inv) => (
              <div key={inv.id} className="p-3.5 rounded-xl bg-slate-900/60 border border-slate-800 flex items-center justify-between text-xs">
                <div>
                  <div className="font-semibold text-slate-200">{inv.email}</div>
                  <div className="text-[10px] text-slate-500">Sent: {inv.sentAt.substring(0, 10)}</div>
                </div>
