  ...
  return (
    <aside className="w-80 h-full border-l border-slate-800/80 bg-slate-950/95 glass-panel p-5 overflow-y-auto z-20 space-y-6">
      ...
      <div className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800 space-y-2">
        ...
        <h2 className="text-sm font-bold text-slate-100">{document.title}</h2>
      ...
      <div className="space-y-2 text-xs">
        <h4 className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Extracted Metadata</h4>
        <div className="p-3 rounded-xl bg-slate-900/40 border border-slate-800/60 space-y-2 font-mono text-[11px]">
          <div className="flex justify-between">
            <span className="text-slate-400">File Size:</span>
            <span className="text-slate-200">{document.fileSizeBytes} Bytes</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-400">Language Confidence:</span>
            <span className="text-emerald-400">English (99.8%)</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-400">Word Count:</span>
            <span className="text-slate-200">1,240 Words</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-400">Reading Time:</span>
            <span className="text-slate-200">3 Minutes</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-400">Checksum (SHA256):</span>
            <span className="text-cyan-400 truncate max-w-[120px]">{document.checksumSha256}</span>
          </div>
        </div>
      </div>
      ...
      {/* Processing Timeline */}
      <div className="space-y-2 text-xs">
        <h4 className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Processing Timeline</h4>
        <div className="space-y-2 pl-2 border-l border-slate-800">
          {timelineSteps.map((step) => (
            <div key={step.name} className="relative pl-4 space-y-0.5">
              ...
              <div className="font-semibold text-slate-200 text-[11px]">{step.name}</div>
              <div className="text-[9px] text-slate-500 font-mono">{step.time}</div>
            </div>
          ))}
        </div>
      </div>
