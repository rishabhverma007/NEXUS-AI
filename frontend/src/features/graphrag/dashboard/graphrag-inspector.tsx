  ...
  return (
    <aside className="w-80 h-full border-l border-slate-800/80 bg-slate-950/95 glass-panel p-5 overflow-y-auto z-20 space-y-6">
      ...
      <div className="space-y-4 text-xs">
        <div>
          <span className="text-[10px] text-slate-500 font-mono block">Identified Seed Nodes</span>
        ...
        <div className="space-y-2">
          <span className="text-[10px] text-slate-500 font-mono block">Discovered Relationship Paths</span>
          {context.paths.map((p) => (
            <div key={p.id} className="p-3 rounded-xl bg-slate-900/60 border border-slate-800 space-y-1 font-mono text-[11px]">
              <div className="text-cyan-400 font-bold">{p.pathChain.join(" → ")}</div>
              <div className="text-[10px] text-slate-400">Path Weight: {p.pathWeight}</div>
            </div>
          ))}
        </div>

        <div className="space-y-2">
          <span className="text-[10px] text-slate-500 font-mono block">Graph Context Payload ({context.estimatedTokens} Tokens)</span>
          <pre className="p-3 rounded-xl bg-slate-950 border border-slate-900 font-mono text-[10px] text-slate-300 overflow-x-auto whitespace-pre-wrap">
